...
//                $proposalProduct->discount    = isset($products[$i]['discount']) ? $products[$i]['discount'] : 0;
    ...
    public function sent($id)
    {
        ...
        if (\Auth::user()->can('send proposal')) {
            ...
            try
            {
                $resp = Utility::sendEmailTemplate('proposal_send', [$customer->id => $customer->email], $uArr);
            }
             catch (\Exception $e) {
    ...
    public function convert($proposal_id)
    {
        if (\Auth::user()->can('convert invoice')) {
            ...
            $proposal->converted_invoice_id = $convertRetainer->id;
            $proposal->save();
    ...
    public function convertInvoice($proposal_id)
    {
        ...
        if(\Auth::user()->can('convert invoice'))
        {
            ...
            $proposal->is_converted_invoice_id = $convertInvoice->id;
            $proposal->save();

...
@push('script-page')
    ...
    <script type="text/javascript">
        @if($retainer->getDue() > 0  && !empty($company_payment_setting) &&  $company_payment_setting['is_stripe_enabled'] == 'on' && !empty($company_payment_setting['stripe_key']) && !empty($company_payment_setting['stripe_secret']))
...
@section('action-btn')
    ...
    @if($retainer->is_convert==0)
        @can('convert invoice')
                ...
                {!! Form::open(['method' => 'get', 'class' => ' btn btn-sm btn-primary align-items-center', 'route' => ['retainer.convert', $retainer->id],'id'=>'proposal-form-'.$retainer->id]) !!}
    ...
    @endif
        ...
        <a href="#" class="btn btn-sm btn-primary  cp_link" data-link="{{route('pay.retainerpay',\Illuminate\Support\Facades\Crypt::encrypt($retainer->id))}}" data-bs-toggle="tooltip" title="{{__('Copy')}}" data-original-title="{{__('Click to copy Retainer link')}}">
                    ...
                    <div class="card-body">
                        <div class="row timeline-wrapper">
                            <div class="col-md-6 col-lg-4 col-xl-4">
                                ...
                                <p class="text-muted text-sm mb-3"><i class="ti ti-clock mr-2"></i>{{__('Created on ')}}{{\Auth::user()->dateFormat($retainer->issue_date)}}</p>
                                @can('edit invoice')
                                    <a href="{{ route('retainer.edit',\Crypt::encrypt($retainer->id)) }}" class="btn btn-sm btn-primary" data-bs-toggle="tooltip" data-original-title="{{__('Edit')}}"><i class="ti ti-edit mr-2"></i>{{__('Edit')}}</a>
                                 @endcan
                            </div>
                            <div class="col-md-6 col-lg-4 col-xl-4">
                                <div class="timeline-icons"><span class="timeline-dots"></span>
                                ...
                                <p class="text-muted text-sm mb-3">
                                    @if($retainer->status!=0)
                                        <i class="ti ti-clock mr-2"></i>{{__('Sent on')}} {{\Auth::user()->dateFormat($retainer->send_date)}}
                                    @else
                                        @can('send invoice')
                                ...
                                @if($retainer->status==0)
                                    @can('send bill')
                                        <a href="{{ route('retainer.sent',$retainer->id) }}" class="btn btn-sm btn-warning" data-bs-toggle="tooltip" data-original-title="{{__('Mark Sent')}}"><i class="ti ti-send mr-2"></i>{{__('Send')}}</a>
                                    @endcan
                                @endif
                            </div>
                            <div class="col-md-6 col-lg-4 col-xl-4">
                                ...
                                @if($retainer->status!=0 && $retainer->is_convert == 0)
                                    @can('create payment invoice')
                                        <a href="#" data-url="{{ route('retainer.payment',$retainer->id) }}" data-ajax-popup="true" data-title="{{__('Add Payment')}}" class="btn btn-sm btn-info" data-original-title="{{__('Add Payment')}}"><i class="ti ti-report-money mr-2"></i>{{__('Add Payment')}}</a> <br>
                                    @endcan
                                @endif
        ...
        @if($retainer->status!=0)
            <div class="row justify-content-between align-items-center mb-3">
                <div class="col-md-12 d-flex align-items-center justify-content-between justify-content-md-end">
                    ...
                    @endif
                    <div class="all-button-box mr-2">
                        <a href="{{ route('retainer.resent',$retainer->id)}}" class="btn btn-sm btn-primary">{{__('Resend Retainer')}}</a>
                    </div>
                    <div class="all-button-box">
                        <a href="{{ route('retainer.pdf', Crypt::encrypt($retainer->id))}}" target="_blank" class="btn btn-sm btn-primary">{{__('Download')}}</a>
                    </div>
                </div>
            </div>
        @endif
        ...
        <div class="row justify-content-between align-items-center mb-3">
            <div class="col-md-12 d-flex align-items-center justify-content-between justify-content-md-end">
                <div class="all-button-box mx-2">
                    <a href="#" class="btn btn-xs btn-primary btn-icon-only width-auto" data-url="{{route('customer.retainer.send',$retainer->id)}}" data-ajax-popup="true" data-title="{{__('Send Invoice')}}">
                        {{__('Send Mail')}}
                    </a>
                </div>
                <div class="all-button-box mx-2">
                        ...
                        <div class="invoice-print">
                            <div class="row invoice-title mt-2">
                                ...
                                <div class="col-xs-12 col-sm-12 col-nd-6 col-lg-6 col-12 text-end">
                                    <h3 class="invoice-number">{{ AUth::user()->retainerNumberFormat($retainer->retainer_id) }}</h3>
                                </div>
                                <div class="col-12">
                                    <hr>
                                </div>
                                ...
                                @endif
                                    <div class="col">
                                        <div class="float-end mt-3">
                                            {!! DNS2D::getBarcodeHTML(route('pay.invoice',\Illuminate\Support\Facades\Crypt::encrypt($retainer->id)), "QRCODE",2,2) !!}
                                        </div>
                                    </div>
                            </div>
                            <div class="row mt-3">
                                <div class="col">
                                    <small>
                                        ...
                                        @if($retainer->status == 0)
                                            <span class="badge fix_badge rounded px-3 p-1 bg-primary ">{{ __(\App\Models\Retainer::$statues[$retainer->status]) }}</span>
                                        @elseif($retainer->status == 1)
                                            <span class="badge fix_badge rounded px-3 p-1 bg-info">{{ __(\App\Models\Retainer::$statues[$retainer->status]) }}</span>
                                        @elseif($retainer->status == 2)
                                            <span class="badge fix_badge rounded px-3 p-1 bg-secondary">{{ __(\App\Models\Retainer::$statues[$retainer->status]) }}</span>
                                        @elseif($retainer->status == 3)
                                            <span class="badge fix_badge rounded px-3 p-1 bg-warning">{{ __(\App\Models\Retainer::$statues[$retainer->status]) }}</span>
                                        @elseif($retainer->status == 4)
                                            <span class="badge fix_badge rounded px-3 p-1 bg-danger">{{ __(\App\Models\Retainer::$statues[$retainer->status]) }}</span>
                                        @endif
                                    </small>
                                </div>
                                    ...
                                    <div class="table-responsive mt-2">
                                        <table class="table mb-0 table-striped">
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-right">{{\Auth::user()->priceFormat($retainer->getSubTotal())}}</td>
                                            </tr>
                                            
                                                <tr>
                                                    <td colspan="6"></td>
                                                    <td class="text-right"><b>{{__('Discount')}}</b></td>
                                                    <td class="text-right">{{\Auth::user()->priceFormat($retainer->getTotalDiscount())}}</td>
                                                </tr>
                                            ...
                                            <tr>
                                                ...
                                                <td class="blue-text text-right">{{\Auth::user()->priceFormat($retainer->getTotal())}}</td>
                                            </tr>
                                            <tr>
                                                <td colspan="6"></td>
                                                <td class="text-right"><b>{{__('Paid')}}</b></td>
                                                <td class="text-right">{{\Auth::user()->priceFormat(($retainer->getTotal()-$retainer->getDue()))}}</td>
                                            </tr>
                                            <tr>
                                                <td colspan="6"></td>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-right">{{\Auth::user()->priceFormat($retainer->getDue())}}</td>
                                            </tr>
                                            </tfoot>
                                        </table>
                                    </div>
                                    ...
                                    <tr>
                                        <td>
                                            @if(!empty($payment->add_receipt))
                                                <a href="{{ $paymentpath . '/' . $payment->add_receipt }}" download="" class="btn btn-sm btn-primary btn-icon rounded-pill" target="_blank"><span class="btn-inner--icon"><i class="ti ti-download"></i></span></a>
                                                <a href="{{ $paymentpath . '/' . $payment->add_receipt }}"  class="btn btn-sm btn-secondary btn-icon rounded-pill" target="_blank"><span class="btn-inner--icon"><i class="ti ti-crosshair"></i></span></a>
                                            @else
                                                -
                                            @endif
                                        </td>

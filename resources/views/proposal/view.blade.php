...
@section('action-btn')
<div class="float-end">
    @if($proposal->is_convert_invoice==0)
        @if($proposal->is_convert==0)
            @can('convert invoice')
                
                    {!! Form::open(['method' => 'get', 'class' => ' btn btn-sm btn-primary align-items-center', 'route' => ['proposal.convert', $proposal->id],'id'=>'proposal-form-'.$proposal->id]) !!}
                    <a href="#" class="bs-pass-para" data-bs-toggle="tooltip" title="{{__('Convert into Retainer')}}" data-original-title="{{__('Convert to Retainer')}}" data-original-title="{{__('Delete')}}" data-confirm="{{__('You want to confirm convert to invoice. Press Yes to continue or Cancel to go back')}}" data-confirm-yes="document.getElementById('proposal-form-{{$proposal->id}}').submit();">
        ...
        @else
            @can('convert invoice')
                    ...
                    <a href="{{ route('retainer.show',\Crypt::encrypt($proposal->converted_invoice_id)) }}" class="btn btn-sm btn-primary align-items-center" data-bs-toggle="tooltip" title="{{__('Already convert to Retainer')}}" >
    ...
    @if($proposal->is_convert_invoice==0)
        @can('convert invoice')
                {!! Form::open(['method' => 'get', 'class' => ' btn btn-sm btn-warning align-items-center', 'route' => ['proposal.convertinvoice', $proposal->id],'id'=>'proposal-form-'.$proposal->id]) !!}
                <a href="#" class="bs-pass-para" data-bs-toggle="tooltip" title="{{__('Convert into Invoice')}}" data-original-title="{{__('Convert to Retainer')}}" data-original-title="{{__('Delete')}}" data-confirm="{{__('You want to confirm convert to invoice. Press Yes to continue or Cancel to go back')}}" data-confirm-yes="document.getElementById('proposal-form-{{$proposal->id}}').submit();">
                    <i class="ti ti-exchange text-white"></i>
                    {!! Form::close() !!}
                </a>
    ...
    @else
        @can('show invoice')
                <a href="{{ route('invoice.show',\Crypt::encrypt($proposal->is_converted_invoice_id)) }}" class="btn btn-sm btn-warning align-items-center" data-bs-toggle="tooltip" title="{{__('Already convert to Invoice')}}" >
                    <i class="ti ti-eye text-white"></i>
                </a>
                        ...
                        <div class="card-body">
                            <div class="row timeline-wrapper">
                                <div class="col-md-6 col-lg-4 col-xl-4">
                                    ...
                                    @can('edit proposal')
                                        <a href="{{ route('proposal.edit',\Crypt::encrypt($proposal->id)) }}" class="btn btn-sm btn-primary" data-bs-toggle="tooltip" data-original-title="{{__('Edit')}}"><i class="ti ti-edit mr-2"></i>{{__('Edit')}}</a>

    ...
    <ul class="list-unstyled">
        ...
        <li class="dropdown dash-h-item drp-company">
            ...
            <span class="theme-avtar">
                ...
                <img src="{{ $profile . (isset(\Auth::user()->avatar) && !empty(\Auth::user()->avatar)? \Auth::user()->avatar:'logo-dark.png')}}" class="img-fluid rounded-circle">
            @else
            ...
            @endif
                ...
                <div class="dropdown-menu dash-h-dropdown">
                    ...
                    @if(\Auth::guard('customer')->check())
                    <a href="{{ route('customer.logout') }}" onclick="event.preventDefault(); document.getElementById('frm-logout').submit();" class="dropdown-item">
                    ...
                    @elseif(\Auth::guard('vender')->check())
                    <a href="{{ route('vender.logout') }}" onclick="event.preventDefault(); document.getElementById('frm-logout').submit();" class="dropdown-item">
                        <i class="ti ti-power"></i>
                            <span>{{__('Logout')}}</span>
                    </a>
                    ...
                    <div class="dropdown notification-icon">
                        ...
                        <div class="dropdown-menu" aria-labelledby="dropdownBookmark">
                                @if(Gate::check('create product & service'))
                                    <a class="dropdown-item" href="#" data-url="{{ route('productservice.create') }}" data-ajax-popup="true"  data-size="lg" data-title="{{__('Create New Product')}}"><i class="ti ti-shopping-cart"></i>{{__('Create New Product')}}</a>
                                @endif
                                @if(Gate::check('create customer'))
                                    <a class="dropdown-item" href="#" data-url="{{ route('customer.create') }}" data-ajax-popup="true" data-size="lg" data-title="{{__('Create New Customer')}}"><i class="ti ti-user"></i>{{__('Create New Customer')}}</a>
                                @endif
                                @if(Gate::check('create vender'))
                                    <a class="dropdown-item" href="#" data-url="{{ route('vender.create') }}" data-ajax-popup="true" data-size="lg" data-title="{{__('Create New Vendor')}}"><i class="ti ti-note"></i>{{__('Create New Vendor')}}</a>
                                @endif
                    ...
                    <li class="dropdown dash-h-item drp-language">
                        ...
                        <div class="dropdown-menu dash-h-dropdown dropdown-menu-end">
                                ...
                                @endif --}}
                                 @if(\Auth::guard('customer')->check())
                                    @foreach($languages as $language)
                                        <a class="dropdown-item @if($language ==  $currantLang ) text-danger @endif" href="{{route('customer.change.language',$language)}}">{{Str::upper($language)}}</a>
                                    @endforeach

                                @elseif(\Auth::guard('vender')->check())
                                    @foreach($languages as $language)
                                        <a class="dropdown-item @if($language ==  $currantLang ) text-danger @endif" href="{{route('vender.change.language',$language)}}">{{Str::upper($language)}}</a>
                                    @endforeach
                                ...
                                @if(\Auth::user()->type=='company')
                                    ...
                                    <a class="dropdown-item text-primary" href="{{route('manage.language',[$currantLang])}}">{{ __('Manage Language') }}</a>
                                @endif
                    </div>
